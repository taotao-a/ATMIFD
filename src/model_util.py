...
class Temporal_Attention(nn.Module):
    def __init__(self, node_embedding_dim, edge_embedding_dim, log_embedding_dim, trace2pod, heads_node=4, heads_edge=4,
                 heads_log=4, dropout=0.1,
                 window_size=16, batch_size=10):
        super(Temporal_Attention, self).__init__()
        self.window_size = window_size
        self.batch_size = batch_size
        self.register_buffer('trace2pod', trace2pod, persistent=False)
    ...
    def forward(self, x_node, x_trace, x_log, mask=False):
        ...
        if mask:
            mask_att = (torch.triu(
                torch.ones(self.window_size, self.window_size, device=x_node.device)) == 1).transpose(0, 1)
        ...
        att_node = torch.concat([att_n.mean(axis=[1, 2], keepdims=True), att_tn.mean(axis=[1, 2], keepdims=True),
                                 att_l.mean(axis=[1, 2], keepdims=True)], dim=1).mean(axis=1, keepdims=True)
        att_edge = torch.concat([att_nn.mean(axis=[1, 2], keepdims=True), att_t.mean(axis=[1, 2], keepdims=True),
                                 att_ln.mean(axis=[1, 2], keepdims=True)], dim=1).mean(axis=1, keepdims=True)
...
class Spatial_Attention(nn.Module):
    def __init__(self, node_embedding_dim, edge_embedding_dim, log_embedding_dim, heads_n2e=4, heads_e2n=4, dropout=0.1,
                 batch_size=10,
                 window_size=16):
    ...
    def forward(self, x_node, x_trace, x_log, node_adj, edge_adj, edge_efea):
        # --- FIX: handle the last (smaller) batch during evaluation ---
        # The adjacency (node_adj/edge_adj) is built with a fixed self.batch_size.
        # When eval/test last batch has real_B < self.batch_size, reshape/adj will mismatch.
        real_B = x_node.size(0)
        if real_B < self.batch_size:
            pad_B = self.batch_size - real_B
            x_node = torch.cat([x_node, x_node.new_zeros((pad_B, *x_node.shape[1:]))], dim=0)
            x_trace = torch.cat([x_trace, x_trace.new_zeros((pad_B, *x_trace.shape[1:]))], dim=0)
            x_log = torch.cat([x_log, x_log.new_zeros((pad_B, *x_log.shape[1:]))], dim=0)
        elif real_B > self.batch_size:
            raise RuntimeError(
                f"Got batch size {real_B} > configured batch_size {self.batch_size}. "
                f"Please keep DataLoader batch_size consistent with args['batch_size']."
            )
        # -------------------------------------------------------------
        ...
        # Keep a safety check (but no spam prints)
        assert node_adj.size(1) == x_trace.size(0), "edge_index/edge_attr mismatch"

        node = self.node2node(node, node_adj, x_trace)
        x_trace = self.egde2node(x_trace, edge_adj, node[edge_efea.long()])

        x_node_out = node[:, :x_node.shape[-1]].reshape(self.batch_size, self.window_size, -1, x_node.shape[-1])
        x_trace_out = x_trace.reshape(self.batch_size, self.window_size, -1, x_trace.shape[-1])
        x_log_out = node[:, x_node.shape[-1]:].reshape(self.batch_size, self.window_size, -1, x_log.shape[-1])

        # Cut padding back to the real batch size
        x_node_out = x_node_out[:real_B]
        x_trace_out = x_trace_out[:real_B]
        x_log_out = x_log_out[:real_B]
        return x_node_out, x_trace_out, x_log_out


class Encoder_Decoder_Attention(nn.Module):
    def __init__(self, node_embedding_dim, edge_embedding_dim, log_embedding_dim, heads_node=4, heads_edge=4,
                 heads_log=4, dropout=0.1):
...
class Encoder(nn.Module):
    def __init__(self, graph, node_embedding, edge_embedding, log_embedding, node_heads, log_heads, edge_heads,
        ...
        for name, value in zip(('node_adj', 'node_efea', 'edge_adj', 'edge_efea'),
        ...
        self.sa_add = nn.ModuleList(
            [AddALL(node_embedding, edge_embedding, log_embedding, dropout) for _ in range(self.L)])
    ...
    def forward(self, e_node, e_edge, e_log):
        ...
        for i in range(self.L):
            e_node, e_edge, e_log = self.sa_add[i](e_node, e_edge, e_log,
                                                   *self.spatial_attention[i](e_node, e_edge, e_log, self.node_adj,
                                                                              self.edge_adj, self.edge_efea))
            e_node, e_edge, e_log = self.ta_add[i](e_node, e_edge, e_log,
                                                   *self.temporal_attention[i](e_node, e_edge, e_log))
...
class Decoder(nn.Module):
    def __init__(self, graph, node_embedding, edge_embedding, log_embedding, node_heads, log_heads, edge_heads,
        ...
        self.cross_attention = nn.ModuleList(
            [Encoder_Decoder_Attention(node_embedding, edge_embedding, log_embedding,
                                       heads_node=node_heads, heads_edge=edge_heads, heads_log=log_heads,
                                       dropout=dropout)
    ...
    def forward(self, d_node, d_edge, d_log, z_node, z_edge, z_log):
        ...
        for i in range(self.L):
            d_node, d_edge, d_log = self.sa_add[i](d_node, d_edge, d_log,
                                                   *self.spatial_attention[i](d_node, d_edge, d_log, self.node_adj,
                                                                              self.edge_adj, self.edge_efea))
            d_node, d_edge, d_log = self.ta_add[i](d_node, d_edge, d_log,
                                                   *self.temporal_attention[i](d_node, d_edge, d_log, mask=True))
            d_node, d_edge, d_log = self.ca_add[i](d_node, d_edge, d_log,
                                                   *self.cross_attention[i](d_node, d_edge, d_log, z_node, z_edge,
                                                                            z_log))
...
class Embed(nn.Module):
    ...
    def forward(self, X):
        ...
        if self.dim == 4:
            ...
            return X + Variable(self.pe[:, :X.shape[1], :, :], requires_grad=False), X_new[:, :X.shape[1], :,
                                                                                     :] + Variable(
