...
class MyModel(nn.Module):
	def __init__(self, graph, **args):
		...
		self.name = 'my'
		# Non-persistent buffers move with the model without changing legacy checkpoint keys.
		self.register_buffer('graph', torch.as_tensor(graph).clone(), persistent=False)
		...
		trace2pod_rec = torch.nn.functional.one_hot(adj[0], num_classes=self.graph.shape[0]) \
			+ torch.nn.functional.one_hot(adj[1], num_classes=self.graph.shape[0])
		self.register_buffer('trace2pod', trace2pod_rec / 2, persistent=False)
