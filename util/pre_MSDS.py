...
def deal_log(data_path):
    config_file = str(Path(__file__).resolve().parent / 'msds.ini')
    template_file = str(Path(Save_Path) / 'log_template.csv')
    state_file = str(Path(Save_Path) / 'log_state.pkl')
...
def preprocess(raw_path, save_path, start_time, end_time, overwrite=False):
    global Raw_Path, Save_Path, start_dealtime, end_dealtime
    root = Path(__file__).resolve().parents[1]
    raw, save = Path(raw_path).expanduser(), Path(save_path).expanduser()
    Raw_Path = str((raw if raw.is_absolute() else root / raw).resolve())
    Save_Path = str((save if save.is_absolute() else root / save).resolve())
    start_dealtime = time.mktime(time.strptime(start_time, '%Y-%m-%d %H:%M:%S'))
    end_dealtime = time.mktime(time.strptime(end_time, '%Y-%m-%d %H:%M:%S'))
    if end_dealtime <= start_dealtime:
        raise ValueError('--end must be later than --start')
    required = [Path(Raw_Path) / 'metrics', Path(Raw_Path) / 'traces',
                Path(Raw_Path) / 'logs' / 'logs_aggregated_concurrent.csv']
    missing = [str(path) for path in required if not path.exists()]
    if missing:
        raise FileNotFoundError('Missing raw MSDS inputs: ' + ', '.join(missing))
    outputs = ['metric.csv', 'log.csv', 'trace.csv', 'trace_path.pkl',
               'log_state.pkl', 'log_template.csv']
    if not overwrite and any((Path(Save_Path) / name).exists() for name in outputs):
        raise ValueError('Output files already exist; choose another --save_path or explicitly use --overwrite')
    TRACERESULT.clear()
    ...
    logger.info('Preprocessing saved to %s. Supply the matching label.pkl separately.', Save_Path)


def main():
    import argparse
    root = Path(__file__).resolve().parents[1]
    parser = argparse.ArgumentParser(description='Preprocess MSDS using the original time alignment')
    parser.add_argument('--raw_path', default=str(root / 'data/MSDS/concurrent_data'))
    parser.add_argument('--save_path', default=str(root / 'data/MSDS-pre'))
    parser.add_argument('--start', default=start)
    parser.add_argument('--end', default=end)
    parser.add_argument('--overwrite', action='store_true')
    options = parser.parse_args()
    preprocess(options.raw_path, options.save_path, options.start, options.end, options.overwrite)
...
if __name__ == '__main__':
    try:
        ...
        raise SystemExit('MSDS preprocessing: {}'.format(error))
